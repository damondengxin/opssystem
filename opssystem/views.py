...
#from opssystem.models import Global_Config,Email_Config
from cmdb.models import  Assets,Log_Assets
from codedeploy.models import Project_Config,Project_Order
from taskmanager.models import  Cron_Config
from appdeploy.models import  Ansible_Playbook
from opssystem.models import Global_Config,Email_Config
...
def index(request):
    #7天更新频率统计
    userList = Project_Order.objects.raw('''SELECT id,order_user FROM opssystem_project_order GROUP BY order_user;''')
    ...
    for user in userList:
        ...
        for startTime in dateList:
            sql = """SELECT id,IFNULL(count(0),0) as count from opssystem_project_order WHERE 
    ...
    for ms in monthList:
        ...
        data['date'] = ms
        for user in userList:
            sql = """SELECT id,IFNULL(count(0),0) as count from opssystem_project_order WHERE date_format(create_time,"%%Y%%m%%d") >= {startTime} and 
...
@login_required(login_url='/login')
@permission_required('opssystem.can_change_global_config', login_url='/noperm/')
def config(request):
