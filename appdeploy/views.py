...
from .models import Ansible_Playbook, Ansible_Playbook_Number, Log_Ansible_Model, Log_Ansible_Playbook
from opssystem.models import Ansible_CallBack_Model_Result,Ansible_CallBack_PlayBook_Result
from cmdb.models import Server_Assets,Assets,Service_Assets
from opssystem.data.DsMySQL import AnsibleRecord
...
def apps_model(request):
    ...
    elif request.method == "POST" and request.user.has_perm('opssystem.can_change_ansible_playbook'):
        resource = []
        sList = []
...
@login_required()
@permission_required('opssystem.can_add_ansible_playbook', login_url='/noperm/')
def apps_add(request):
    if request.method == "GET":
        serverList = Server_Assets.objects.all()
        groupList = Group.objects.all()
        userList = User.objects.all()
        return render(request,'apps/apps_playbook_config.html', {"user": request.user, "userList": userList,
                                                                     "serverList": serverList, "groupList": groupList})
        ...
        try:
            ...
            return render(request,'apps/apps_playbook_config.html',
...
@login_required()
@permission_required('opssystem.can_read_ansible_playbook', login_url='/noperm/')
...
def apps_playbook_modf(request, pid):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return render(request,'apps/apps_playbook_modf.html',
                                      {"user": request.user, "errorInfo": "剧本添加错误：%s" % str(e)})
        if request.POST.getlist('playbook_server', None):
