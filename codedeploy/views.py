...
from .models import  Project_Config, Project_Number, Project_Order, Log_Project_Config
from cmdb.models import  Server_Assets
from opssystem.utils.git import GitTools
from opssystem.utils.svn import SvnTools
from opssystem.utils import base
from opssystem.data.DsRedisOps import DsRedis
from opssystem.utils.ansible_api_v2 import ANSRunner
from django.contrib.auth.models import User, Group
from django.db.models import Count
from django.db.models import Q
from opssystem.tasks import recordProject, sendEmail
...
def deploy_modf(request, pid):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            recordProject.delay(project_user=str(request.user), project_id=pid, project_name=project.project_name,
...
def deploy_run(request, pid):
    ...
    if request.method == "GET":
        ...
        if DsRedis.OpsProject.get(redisKey=project.project_uuid + "-locked") is None:  # 判断该项目是否有人在部署
                ...
                if project.project_local_command:
                    ...
                    DsRedis.OpsDeploy.lpush(project.project_uuid,
                                            data="[Running] Execute local command: {cmds} info: {info}".format(
                                                cmds=project.project_local_command, info=result[1]))
                    if result[0] > 0: return JsonResponse({'msg': result[1], "code": 500, 'data': []})
                    # 非编译型语言
                else:
                    # 获取要排除的文件
            ...
            DsRedis.OpsDeploy.lpush(project.project_uuid,
                                    data="[Running] Chown {user} to {path}".format(user=project.project_user,
                                                                                   path=trueDir))
            if result[0] > 0: return JsonResponse({'msg': result[1], "code": 500, 'data': []})
            ...
            return JsonResponse({'msg': "项目部署失败：{user}正在部署改项目，请稍后再提交部署。".format(
                user=DsRedis.OpsProject.get(redisKey=project.project_uuid + "-locked")), "code": 500, 'data': []})
...
def deploy_ask(request, pid):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return render(request,'deploy/deploy_ask.html',
                                      {"user": request.user, "errorInfo": "项目部署申请失败：%s" % str(e)})
...
def deploy_order_status(request, pid):
    if request.method == "GET":
        ...
        return render(request,'deploy/deploy_order_status.html',
                                  {"user": request.user, "order": order, "serverList": serverList})
